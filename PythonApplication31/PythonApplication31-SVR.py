# experiment04 - 200 users each time, Movie Domain, increasing training set, 10 test sets, 10 sets for user - SVR Rregressor. Parameters: kernel='rbf', C=1, gamma=0.1, epsilon= 0.1 
#All 200 (or  x) users in one running!!! Creating One worksheet per user + a file with the TOTAL Average Error across the 200 (or  x) users.
import pandas as pd
import numpy as np
...
workbook_totals = xlsxwriter.Workbook('C:/bear/MovieLens Dataset/Data files-EXPERIMENT04/EXPERIMENT04-all users/Results-project31 tests/SVRMoviesDomain/SVRusers ' + str(min_user_number) + '-' + str(max_user_number - 1) + ' Totals-n.xlsx')
...
for user_num1 in range(min_user_number, max_user_number):
    #Results file
    workbook = xlsxwriter.Workbook('C:/bear/MovieLens Dataset/Data files-EXPERIMENT04/EXPERIMENT04-all users/Results-project31 tests/SVRMoviesDomain/SVRMoviesDomainUser' + str(user_num1) + '-n.xlsx')
    ...
    worksheets[number_of_test_sets].write(0, 1, "SVR Average error 10 test sets")
    worksheets_totals[0].write(0, user_num1 - min_user_number + 1, "SVR Av. error user "+str(user_num1))
 ...
 #   print('user_num = ',user_num)
    ...
    #    while user_num < number_of_users: 
        ...
        while testset_number < number_of_test_sets:
            ...
            worksheets[testset_number].write(0, 1, "SVR Average error for 10 tests per a test set")
            worksheets[testset_number].write(0, 2, "SVR Average error for test 1")
            worksheets[testset_number].write(0, 3, "SVR Average error for test 2")
            worksheets[testset_number].write(0, 4, "SVR Average error for test 3")
            worksheets[testset_number].write(0, 5, "SVR Average error for test 4")
            worksheets[testset_number].write(0, 6, "SVR Average error for test 5")   
            worksheets[testset_number].write(0, 7, "SVR Average error for test 6")
            worksheets[testset_number].write(0, 8, "SVR Average error for test 7")
            worksheets[testset_number].write(0, 9, "SVR Average error for test 8")
            worksheets[testset_number].write(0, 10, "SVR Average error for test 9")
            worksheets[testset_number].write(0, 11, "SVR Average error for test 10")
